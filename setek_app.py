...
# 4. 화면 구성 및 사이드바
# ==========================================
st.title("📝 NEIS 세특 AI 어시스턴트 (V40: 보고서 기반 평가 명확화)")
...
if st.button("🚀 세특 초안 생성하기", type="primary", use_container_width=True):
    if not subject:
        ...
        with st.spinner("3/4: 입체적 뼈대 설계 중..."):
            ...
            # 💡 V40 뼈대 수정: 보고서 내용 기반 탐구 역량 평가 명시
            bp_prompt = f"""
            당신은 최고의 고등학교 {subject} 교사입니다.
            [관찰 가능한 행동 동사] 내면 상태('이해함', '체득함') 절대 금지. 능동적 행동('증명함', '분석함')으로 우회하세요.
            [동기-과정-결과-평가 흐름] 1.활동 동기 -> 2.탐구 과정 -> 3.결과 도출 -> 4.보고서 내용 기반 탐구 역량 평가(없으면 AI가 직접 평가)의 순서로 인과관계가 자연스럽게 이어지도록 서술하세요. 과정 중간에 "이는 ~ 핵심 과정임" 같은 메타 평가는 금지합니다.
            [공통 가이드라인] {guidelines}
        ...
        with st.spinner("4/4: 수식 차단 및 문맥 꼬리물기 서술 중..."):
            ...
            # 💡 V40 최종 생성 프롬프트 수정: 평가의 범위 한정 및 중간 요약 금지
            prompt = f"""
            ...
            [🔥 V40 최우선 엄수 규칙: 보고서 기반 평가 한정 및 서사 최적화 🔥]
            1. 동기-과정-결과-평가의 완벽한 흐름 (보고서 기반 평가 명확화): [활동 동기] ➡️ [구체적 탐구 과정] ➡️ [결과 도출] ➡️ [보고서 내용 기반 탐구 역량 평가] ➡️ [인지적/인성 종합 평가] 순서로 자연스러운 꼬리물기 흐름을 만드세요. 이 때 '탐구 역량 평가'는 오직 학생이 수행한 보고서 내용과 결과물에 한정된 학업적/논리적 평가여야 합니다. 교사의 '보고서 역량 평가' 입력이 비어있다면, AI가 임의로 지어내지 말고 보고서 팩트를 분석하여 역량을 직접 도출해 건조하게 평가하세요.
...
if st.session_state.current_result:
    ...
    st.divider()
    st.subheader("🎯 생성된 맞춤형 세특 (보고서 평가 명확화 적용)")
