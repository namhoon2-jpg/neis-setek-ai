...
# 4. 화면 구성 및 사이드바
# ==========================================
st.title("📝 NEIS 세특 AI 어시스턴트 (V33: 어미 문법 완벽 교정판)")
...
if st.button("🚀 세특 초안 생성하기", type="primary", use_container_width=True):
    if not subject:
        ...
        with st.spinner("4/4: 수식 추상화 및 팩트 기반 서술 중..."):
            ...
            [🔥 V33 최우선 엄수 규칙: 문법 교정 및 멸균 🔥]
            1. 간결한 호흡 (만연체 금지): 한 문장이 너무 길어지지 않도록, 팩트 단위로 문장을 적절히 끊어서 서술하세요.
            2. 자연스러운 시작: 절대 '과목에서', '본 과목에서는' 같은 말로 시작하지 마세요. 바로 활동 내용으로 직행하세요.
            3. 수식 절대 작성 금지 (추상화 요약): 수식이나 공식을 한글 발음(예: 엘원 코사인 세타...)으로 절대 적지 마세요. "삼각함수를 활용하여 로봇팔의 좌표를 구하는 수학적 원리를 도출함"과 같이 '원리와 목적'만 우회하여 압축하세요.
            ...
            8. 능동태 및 자연스러운 음슴체 (매우 중요): '이해함', '느낌' 금지. 문장 끝은 명사형 종결어미로 끝나야 하지만, '모습을함', '태도를함', '자신감함' 처럼 명사에 억지로 '함'을 붙이는 문법 파괴를 절대 하지 마세요. 반드시 '모습을 보임', '자신감을 가짐', '자신감을 보임'처럼 자연스러운 동사를 활용하여 마무리하세요.
            """
...
if st.session_state.current_result:
    ...
    # 💡 V33 핵심: 금지어 및 어색한 어미 파이썬 레벨 교정망 추가
    forbidden_replacements = {
        ...
        "자신감함": "자신감을 보임", "자신감을함": "자신감을 보임", "자신감을 함": "자신감을 보임",
        "자신감임": "자신감을 보임"
    }
    ...
    st.divider()
    st.subheader("🎯 생성된 맞춤형 세특 (어미 문법 파괴 방지 적용)")
