...
with st.sidebar:
    ...
    # 💡 2015/2022 교육과정 선택 추가
    curriculum = st.radio("교육과정", ["2015 개정 교육과정", "2022 개정 교육과정"], index=1)
...
if st.button("🚀 세특 초안 생성하기", type="primary", use_container_width=True):
    if not subject:
        ...
        with st.spinner(f"0/4: {curriculum} 성취기준 탐색 중..."):
            # 💡 프롬프트에 선택된 교육과정 변수 적용
            edu_prompt = f"고등학교 '{subject}' 과목의 {curriculum} 핵심 성취기준과 학습 목표를 3문장 이내로 요약해줘. 세특 작성 가이드라인으로 쓸거야."
            try:
        ...
        with st.spinner("4/4: 교육과정 연계 및 서사 융합 서술 중..."):
            ...
            # 💡 V43 본 프롬프트: AI 특유의 나열, 끊어치기, 거창한 표현 완벽 금지 (교육과정 변수 적용)
            prompt = f"""
            ...
            3. 교육과정 부합성 (명칭 직접 노출 절대 금지): 서술 시 [{curriculum} 성취기준]의 핵심 개념을 자연스럽게 녹여내되, "{curriculum}"이라는 단어 자체는 세특에 절대 적지 마세요.
...
if st.session_state.current_result:
    ...
    # 💡 금지어 사전: AI 특유의 찌꺼기 및 선택된 교육과정 단어 완벽 차단
