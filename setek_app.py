...
# 4. 화면 구성 및 사이드바
# ==========================================
st.title("📝 NEIS 세특 AI 어시스턴트 (V32: 문법 교정 & 만연체 방지)")
...
if st.button("🚀 세특 초안 생성하기", type="primary", use_container_width=True):
    if not subject:
        ...
        with st.spinner("3/4: 입체적 뼈대 설계 중..."):
            ...
            [관찰 가능한 행동 동사] 내면 상태('이해함', '체득함', '깨달음') 절대 금지. 능동적 행동('증명함', '분석함')으로 우회하세요.
            [공통 가이드라인] {guidelines}
        ...
        with st.spinner("4/4: 수식 추상화 및 팩트 기반 서술 중..."):
            ...
            # 💡 V32 핵심 1: 프롬프트에 만연체 금지 및 자연스러운 시작/어미 규칙 추가
            prompt = f"""
            ...
            [🔥 V32 최우선 엄수 규칙: 문법 교정 및 멸균 🔥]
            1. 간결한 호흡 (만연체 금지): 한 문장이 너무 길어지지 않도록, 팩트 단위로 문장을 적절히 끊어서 서술하세요. 문장이 길어지면 가독성이 떨어집니다.
            2. 자연스러운 시작: 글을 시작할 때 절대로 '과목에서', '본 과목에서는' 같은 어색한 말로 시작하지 마세요. 바로 '~에 호기심을 가져'처럼 활동 내용으로 직행하세요.
            3. 수식 절대 작성 금지 (추상화 요약): 수식이나 공식을 한글 발음(예: 엘원 코사인 세타...)으로 절대 적지 마세요. "삼각함수를 활용하여 로봇팔의 끝점 좌표를 계산하는 원리를 도출함"과 같이 '원리와 목적'만 문장으로 우회하여 압축하세요.
            4. 분량 및 팩트 보존: 글자 수를 무조건 **최소 {min_c}자 이상, {safe_max_c}자 이하**로 풍성하게 꽉 채우세요.
            5. 마크다운 및 기호 전면 금지: 별표(**), 샵(#), 숫자 번호, 줄바꿈을 절대 사용하지 마세요. 평문(Plain text) 덩어리로만 작성하세요.
            6. 실명 및 제목/주어 삭제: PDF에 이름이 등장해도 절대 쓰지 마세요. '제목:', '과목명:', '학생은' 등을 쓰지 마세요.
            7. 자연스러운 4단 흐름: [활동 계기] ➡️ [탐구 과정] ➡️ [결과] ➡️ [교사의 인지적/인성 평가] 순으로 이어지게 하세요.
            8. 능동태 및 자연스러운 음슴체: '이해함', '느낌' 금지. 문장 끝은 명사형 종결어미(~함, ~임, ~보임 등)로 자연스럽게 끝내되, '모습을함', '태도를함' 같은 문법 파괴는 피하세요.
            """
...
if st.session_state.current_result:
    ...
    st.divider()
    st.subheader("🎯 생성된 맞춤형 세특 (문법 교정 및 만연체 방지 적용)")
